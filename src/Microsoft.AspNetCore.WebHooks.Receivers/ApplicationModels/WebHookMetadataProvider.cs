...
namespace Microsoft.AspNetCore.WebHooks.ApplicationModels
{
    ...
    public class WebHookMetadataProvider : IApplicationModelProvider
    {
        ...
        public WebHookMetadataProvider(IEnumerable<IWebHookMetadata> metadata, ILoggerFactory loggerFactory)
        {
            ...
            // Check for IWebHookBodyTypeMetadata services.
        ...
        private void Apply(ActionModel action)
        {
            ...
            // Override the WebHookBodyType if the WebHookAttribute provides that information.
        ...
        protected void EnsureValidBodyTypeMetadata(IReadOnlyList<IWebHookBodyTypeMetadataService> bodyTypeMetadata)
        {
            ...
            foreach (var receiverBodyTypeMetadata in bodyTypeMetadata)
            {
                ...
                if ((~WebHookBodyType.All & receiverBodyTypeMetadata.BodyType) != 0)
                {
            ...
            if (invalidRegistrations)
            {
                var message = string.Format(
                    ...
                    Resources.MetadataProvider_InvalidMetadataServices,
                    typeof(IWebHookBodyTypeMetadataService),
                    nameof(IWebHookBodyTypeMetadataService.BodyType));
        ...
        /// <summary>
        /// Ensure given <paramref name="metadata"/> does not implement <see cref="IWebHookBodyTypeMetadata"/>.
        /// Metadata services should instead implement <see cref="IWebHookBodyTypeMetadataService"/>.
        /// </summary>
        /// <param name="metadata">The collection of <see cref="IWebHookMetadata"/> services.</param>
        protected void EnsureValidBodyTypeMetadata(IEnumerable<IWebHookMetadata> metadata)
        {
            ...
            foreach (var typeName in nonServiceTypeNames)
            {
                ...
                _logger.LogCritical(
                    1,
                    "'{ConcreteType}' implements '{MetadataType}', not all of '{ServiceMetadataType}'.",
                    typeName,
            ...
            IWebHookBodyTypeMetadataService receiverBodyTypeMetadata)
        {
            ...
            switch (actionBodyTypeMetadata.BodyType)
            {
                ...
                case WebHookBodyType.Json | WebHookBodyType.Xml:
                    {
                        // Value contains an invalid combination of flags.
                        _logger.LogCritical(
                            3,
                        ...
                        var message = string.Format(
                            CultureInfo.CurrentCulture,
                            Resources.General_InvalidBodyType,
                            actionBodyTypeMetadata.BodyType,
                            nameof(WebHookBodyType),
                            WebHookBodyType.All);
                        throw new InvalidOperationException(message);
